...
DEBIAN_FRONTEND=noninteractive apt-get install -y python3 python3-pip python3-venv nginx openssl git
...
openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
  -keyout /etc/ssl/private/quiz-app.key \
  -out /etc/ssl/certs/quiz-app.crt \
...
echo "==> Setup complete. App running at https://${app_public_ip}"
